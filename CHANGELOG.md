...
- Conformance fixtures `009-content-blocks-text-only-equivalence` through `017-content-blocks-system-message-text-only` (llm-provider), covering text-only equivalence with the string form, URL-image and inline-base64 image mapping, the `detail` hint, mixed-order preservation, empty-sequence and empty-text-block validation, unsupported-by-model error routing, and the user-only restriction.
