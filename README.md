...
| [0067](proposals/0067-observability-genai-metrics.md) | Draft | spec/observability/spec.md (new §11 *Metrics* — opt-in OA-namespaced `openarmature.gen_ai.client.token.usage` + `…operation.duration` histograms; renumber §11 *Out of scope* → §12); spec/conformance-adapter/spec.md (§6 in-memory metric-capture harness primitive) | Adds the OTel metrics signal, complementing the spans (§4–§6) and logs (§7): two opt-in histograms — token usage and operation duration — recorded per LLM completion and embedding call from the existing §5.5.7 / §5.5.9 typed events. Upstream GenAI metric instruments are Development-status, so OA emits `openarmature.gen_ai.client.*` parallels mirroring upstream type / unit / bucket advisory for a mechanical future cutover (the §5.5.3.1 pattern); only the Stable `error.type` dimension is used directly. Streaming, server, graph-level, and rerank metrics are out of scope (forward pointers to 0062 / 0060). MINOR. |
