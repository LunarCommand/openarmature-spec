...
| [0065](proposals/0065-pipeline-utilities-failure-isolation-cause-fidelity.md) | Draft | spec/pipeline-utilities/spec.md (§6.3 — `caught_exception` gains a carrier-wrapper unwrap clause: at the §9.7 instance and §11.7 branch wrapping sites, `caught_exception.category` MUST reflect the originating `__cause__` category, mirroring §6.1's classifier unwrap; `message` SHOULD track the same cause; a wrapped-lineage SHOULD-clause) | Tightens 0050's failure-isolation event so it reports the *originating* failure category instead of the masking graph-engine §4 `node_exception` wrapper, when `FailureIsolationMiddleware` runs at the §9.7 instance-middleware or §11.7 branch-middleware sites (subgraph boundaries, where the engine wraps the cause before the outer middleware catches it). A consistency fix: §6.1's default retry classifier already unwraps that carrier wrapper to find the real category — the isolation event reporting the wrapper while retry sees through it is the inconsistency 0065 closes. Category fidelity is a MUST (cheap — exception inspection only); the related lineage symptom (instance/branch identity) is a SHOULD where recoverable, since it may need an engine change. Node-level placement and the catch/degrade behavior are unchanged. |
