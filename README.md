...
**Current spec version:** [v0.30.0](CHANGELOG.md)
...
| [0039](proposals/0039-observability-caller-supplied-invocation-id.md) | Draft | observability §5.1 + §8.4.1, graph-engine §3 | Caller-supplied `invocation_id` — opt-in (mirrors `correlation_id`); non-UUID values get a deterministic Langfuse `trace.id` derivation |
