...
| [pipeline-utilities](spec/pipeline-utilities/spec.md) | 0.5.0 | 0.57.0 | 65 | Middleware (canonical retry + timing + failure isolation), parallel fan-out (per-instance resume with success/error discrimination + state migration), checkpointing (shares persistence with suspension), parallel branches. |
...
| [0069](proposals/0069-pipeline-utilities-fan-out-degrade-refinements.md) | Draft | spec/pipeline-utilities/spec.md (§9.3 — three refinements to 0066's fan-out degrade rules: an omitted `extra_outputs` source contributes a null *value* in instance-index order (supersedes 0066's "not contributed / branch-skip" clause); a SHOULD that a non-degrade `instance_middleware` return covers `collect_field`, with an absent `collect_field` on any fan-in path a null slot and never a raise; plus a degrade+resume conformance fixture) | Consolidated follow-up to 0066 from the v0.14.0 implementation review. Corrects the omitted-`extra_outputs` rule from "not contributed" to a **null value** (mirroring `collect_field`'s null, index-aligned with `target_field` under extending reducers); generalizes §9.8's degrade-never-raises so an absent `collect_field` is a graceful null slot on **every** fan-in path, with a SHOULD that non-degrade returns cover it; and adds a conformance fixture pinning that a degraded instance's slot survives a checkpoint+resume round-trip. Supersedes only 0066's `extra_outputs`-omit clause; the 0066 model, §9.8 compile check, and §11.7 branch skip stand. MINOR. |
| [0070](proposals/0070-conformance-adapter-crash-injection-and-cause-chaining.md) | Draft | spec/conformance-adapter/spec.md (§5.6 — formalize the crash/resume directives `first_run_expected_error` / `resume`, and add **`crash_injection`** [crash at a checkpoint boundary, independent of an instance failure]; §5.8 — formalize `saved_record_assertions` / `fan_out_progress` + the `instances_executed`/`skipped_during_resume` assertions; §5.1 — add an optional **`cause`** [exception-chaining] to failure mocks) | Fills the 0055 §5 gap the v0.14.0 review surfaced: documents the crash→save→resume directive vocabulary the checkpoint fixtures already use, and adds two new adapter directives — **`crash_injection`** (test resume from *any* saved state, e.g. a degraded fan-out instance's slot surviving a crash — unblocks 0069's degrade+resume fixture) and mock **`cause`** (a failure mock raises a chained exception — unblocks 0068's outermost-wins cause-chain fixture). The failure-mock family itself (five organic `flaky*` variants) is deferred to a follow-on. MINOR; test-vocabulary only. |
| [0071](proposals/0071-conformance-adapter-failure-mock-catalog.md) | Draft | spec/conformance-adapter/spec.md (§5.1 — document the failure-mock node directives `flaky` [sequence + compact], `flaky_by_index`, `flaky_per_index`, `flaky_instance_only`, `flaky_resume_aware`, and the `failure_sequence` entry; sharpen the confusable `flaky_per_index` (invocation-keyed) vs `flaky_by_index` (attempt / deterministic) pair; flag the inconsistent success-state field naming) | The follow-on 0070 deferred: a faithful catalog of the five failure-mock directives the retry / failure-isolation / checkpoint-resume fixtures already use but §5.1 never documented (only `raises:` was specified). Descriptive — no behavior change, no fixture changes. Keeps the directives **explicit and named** (rejecting one parameterized `flaky` — a self-describing name beats a config blob); flags, without changing, the `success_update` / `on_success` / `success_compute` naming drift. MINOR. |
