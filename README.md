...
| [pipeline-utilities](spec/pipeline-utilities/spec.md) | 0.5.0 | 0.55.0 | 63 | Middleware (canonical retry + timing + failure isolation), parallel fan-out (per-instance resume with success/error discrimination + state migration), checkpointing (shares persistence with suspension), parallel branches. |
...
| [0066](proposals/0066-pipeline-utilities-fan-out-degrade-contribution.md) | Draft | spec/pipeline-utilities/spec.md (§9.3 — a degraded fan-out instance is a §9.3 success whose contribution *is* its `degraded_update`, with `collect_field` + `extra_outputs` read from it by subgraph field name; §9 — a compile-time check that a static `degraded_update` covers `collect_field`, error `fan_out_degraded_update_missing_collect_field`; §11.4 / §11.7 — confirm the heterogeneous branch skip) | Settles what a `FailureIsolation`-degraded fan-out instance contributes to its homogeneous collection slot. A degraded instance is a §9.3 *success* whose contribution **is** the `degraded_update` (read `collect_field` + `extra_outputs` from it by subgraph field name — not merged onto pre-failure state). Guards the slot footgun at **compile time** (a static `degraded_update` must cover `collect_field`, else `fan_out_degraded_update_missing_collect_field`) while keeping the runtime degrade path graceful (a callable that omits it yields a null slot, never a raise — a runtime raise would turn isolation into a graph stop). Confirms the heterogeneous parallel-branches *skip* as the deliberate counterpart. |
