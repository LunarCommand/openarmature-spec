...
| [observability](spec/observability/spec.md) | 0.7.0 | 0.69.0 | 98 | OpenTelemetry + Langfuse backend mappings (LLM completion spans + Langfuse Generation observations; embedding spans + Langfuse dedicated `Embedding` observations; tool-execution spans + Langfuse dedicated `Tool` observations), GenAI metrics, cross-backend correlation IDs, caller-supplied invocation metadata (`openarmature.user.*` span attributes + symmetric `get_invocation_metadata()` read), provider-payload + GenAI semconv attributes (default-off payload via `disable_provider_payload`), prompt-identity / prompt-group linkage, queryable observer pattern, suspension status mapping. |
