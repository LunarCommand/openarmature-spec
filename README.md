...
| [pipeline-utilities](spec/pipeline-utilities/spec.md) | 0.5.0 | 0.59.0 | 68 | Middleware (canonical retry + timing + failure isolation), parallel fan-out (per-instance resume with success/error discrimination + state migration), checkpointing (shares persistence with suspension), parallel branches. |
...
|---|---|---|---|
| [0072](proposals/0072-prompt-management-fetch-cache-ttl.md) | Draft | spec/prompt-management/spec.md (§5 `PromptBackend.fetch` + §6 `PromptManager.fetch` / `get` gain an optional `cache_ttl_seconds`; §5 backend-caching paragraph amended; §15 note); spec/conformance-adapter/spec.md (§6 caching prompt-backend harness primitive) | A per-fetch cache-TTL control for prompt-management: `cache_ttl_seconds` absent = current behavior, `0` forces a fresh read past a backend's client-side cache, `N` bounds staleness. A read-side contract (force-fresh guarantees a fresh *read*, not a cache update; write / eviction stay implementation-defined); non-caching backends no-op it; `render` is unchanged; it composes through the §9 fallback chain. Turns §5's existing "backends MAY cache … invalidation is implementation-defined" into a defined caller lever for on-demand prompt refresh without a restart. MINOR. |
