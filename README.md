...
**Current spec version:** [v0.42.0](CHANGELOG.md)
...
| [0051](proposals/0051-langfuse-trace-io-deprecation-caveat.md) | Draft | observability §8.4.1 *Trace input/output sourcing* | Langfuse trace input/output implementation-surface caveat — documents vendor-SDK deprecation of `set_current_trace_io` while keeping the §8.4.1 normative contract stable across SDK migrations |
| [0052](proposals/0052-implementation-attribution-rows.md) | Draft | observability §5.1 invocation span + §8.4.1 Langfuse mapping | Implementation attribution attributes — `openarmature.runtime.*` invocation-span attributes (and Langfuse trace.metadata rows) carrying implementation name + version (e.g., `openarmature-python` + package version) for cross-impl span attribution |
| [0053](proposals/0053-shared-parent-boundary-clarification.md) | Draft | observability §3.4 *Shared-parent boundary* | §3.4 shared-parent boundary clarification — adds an `outermost_serial` predicate so the invocation span is updated when the invocation's outermost serial wrapper IS the augmenter's call-stack ancestor (vs. true shared-parent cases where it must NOT be) |
