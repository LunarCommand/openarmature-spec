...
| [0033](proposals/0033-prompt-management-surface-refinements.md) | Draft | prompt-management §3 + §4 + §5 + §6 + §7 (new) + §12 | Prompt-management surface refinements — typed `Prompt.sampling` sub-record mirroring `RuntimeConfig`, `LabelResolver` primitive for deployment-time A/B label override, informative filesystem sidecar convention for sampling config, and cross-spec wiring touchpoint to llm-provider §6 |
