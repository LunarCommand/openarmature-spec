...
|---|---|---|---|
| [0079](proposals/0079-retrieval-provider-openai-compatible-embeddings.md) | Draft | spec/retrieval-provider/spec.md §8 *Wire-format mappings* — adds §8.3 OpenAI-compatible embeddings (reuses 0077's §8 + `input_type`) | `base_url`-configurable OpenAI-compatible `/v1/embeddings`: one mapping covers OpenAI + vLLM / TEI / LocalAI / Together. Symmetric (verified: no `input_type` on the wire), exercising 0077's `input_type`-absent degradation; embeddings-only; optional 0077 §8.1 client-side prefix for asymmetric models behind a compatible endpoint. |
