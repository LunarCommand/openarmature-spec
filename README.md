...
|---|---|---|---|
| [0101](proposals/0101-malformed-usage-counter-llm-observability.md) | Draft | llm-provider §6/§7/§8 · graph-engine §6 · observability §5.5.3/§11.2 | The same rule (see accepted 0100) for llm-provider's always-present usage record, plus the observability guard chain a null counter runs through — the OTel usage attributes, the token histogram, and the token-budget instruments all omit rather than emit/compare/divide over a not-reported counter |
