...
|---|---|---|---|
| [0081](proposals/0081-conformance-adapter-value-matcher-vocabulary.md) | Draft | spec/conformance-adapter/spec.md (§5.10 *Value matchers* — normative enumeration of the fixture matcher vocabulary; §3.2 placeholder list cites it) | Promotes the value-matcher vocabulary — inline tokens (`<uuid>` / `<any-string>` / `<trace_id_X>`), assertion sub-keys (`non_empty_string` / `harness_parameterized`), and the exact-value+derivation idiom — from a §3.2 worked example + fixture prose into a normative §5.10 enumeration so every adapter implements one defined set; pins `<any-string>` = non-empty. |
