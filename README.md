...
| [llm-provider](spec/llm-provider/spec.md) | 0.4.0 | 0.20.0 | 31 | Stateless LLM-provider abstraction with canonical error categories, image content blocks for user messages, structured output via `response_schema`, deterministic tool-calling control via `tool_choice`, and a wire-format-mapping catalog (§8.1 OpenAI-compatible; in-spec default for cross-language provider mappings) |
