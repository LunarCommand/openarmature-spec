...
**Current spec version:** [v0.37.0](CHANGELOG.md)
...
| [0046](proposals/0046-prompt-management-multi-message-rendering.md) | Draft | spec/prompt-management/spec.md (§3, §5, §6, §8, §11, §12) | Multi-message / chat prompt rendering — adds a Chat-prompt variant to `Prompt` (role-tagged segments + variable-length placeholder slots for message-list injection); content segments support either a text template or a content-blocks template mirroring llm-provider §3.1 for multimodal user-message authoring (text + image, user-only per §3.1.2); render produces a multi-message `PromptResult` with per-segment / per-block strict-undefined, empty-segment errors, and role-block compatibility enforcement; §6 Text-prompt clause narrowed to exactly one text Message (multi-message / multimodal go through `chat_template`) |
