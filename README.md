...
| [0032](proposals/0032-llm-provider-runtime-config-refinements.md) | Draft | llm-provider §6 + observability §5.5.2 | RuntimeConfig surface refinements — promote `frequency_penalty` / `presence_penalty` / `stop` to declared fields, formalize the extras pass-through contract, mandate null-skip on declared fields, and extend §5.5.2 with the three new GenAI semconv attributes |
