...
|---|---|---|---|
| [0076](proposals/0076-tool-call-request-observability-llm-spans.md) | Draft | spec/observability/spec.md (§5.5 — first-class request-side tool-call attributes on the `openarmature.llm.complete` span: `openarmature.llm.tool_calls.count` / `.names` / `.ids`; §5.5.1 forecast retired) | Surfaces the tool calls a model *requests* in its completion as first-class, queryable span attributes (count / names / ids), promoting them out of the gated free-form output payload (§5.5.1). Names and ids are identity, not payload — un-gated by `disable_provider_payload`, so tool-request visibility survives the default payload-off posture; arguments stay in `output.content`. The *request*-side complement to 0063's tool-*execution* span, split by namespace (`openarmature.llm.tool_calls.*` vs `openarmature.tool.*`) and linked by `ToolCall.id`. OA-namespace (no flat upstream `gen_ai.*` equivalent — the 0050 precedent). MINOR, observability-only. |
| [0067](proposals/0067-observability-genai-metrics.md) | Draft | spec/observability/spec.md (new §11 *Metrics* — opt-in OA-namespaced `openarmature.gen_ai.client.token.usage` + `openarmature.gen_ai.client.operation.duration` histograms; renumber §11 *Out of scope* → §12); spec/conformance-adapter/spec.md (§6 in-memory metric-capture harness primitive) | Adds the OTel metrics signal, complementing the spans (§4–§6) and logs (§7): two opt-in histograms — token usage and operation duration — recorded per LLM completion and embedding call from the existing §5.5.7 / §5.5.9 typed events. Upstream GenAI metric instruments are Development-status, so OA emits `openarmature.gen_ai.client.*` parallels mirroring upstream type / unit / bucket advisory for a mechanical future cutover (the §5.5.3.1 pattern); only the Stable `error.type` dimension is used directly. Streaming, server, graph-level, and rerank metrics are out of scope (forward pointers to 0062 / 0060). MINOR. |
