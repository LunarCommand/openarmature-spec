...
**Current spec version:** [v0.41.0](CHANGELOG.md)
...
| [0050](proposals/0050-retry-and-degradation-primitives.md) | Draft | pipeline-utilities §10 (new §10.X failure-isolation + §10.Y call-level retry) | Retry & degradation primitives — failure-isolation middleware (per-node fallback / continue-on-error / soft-failure surfacing) + LLM call-level retry middleware (transient-classifier + backoff schedule); two-level retry-lane separation guidance |
