...
| A failed call's `error_message` | yes | The Langfuse observer's `disable_provider_payload` covers the harvested exception text, so the default posture withholds it. Full exception detail still reaches your OpenTelemetry backend through `record_exception`. When the flag permits it, the message is also subject to the observer's payload byte cap (§5.5.5 / §8.7), and the Langfuse observer's own cap governs. |
