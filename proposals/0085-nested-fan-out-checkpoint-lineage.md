...
A new pipeline-utilities fixture exercises nested-fan-out resume: an outer fan-out over N items, each outer instance running an inner fan-out, with a crash injected while one outer instance is `in_flight` and another has `completed` its inner fan-out. On resume it asserts (1) the saved `fan_out_progress` holds distinct lineage-qualified entries (not one colliding entry), (2) the re-entering outer instance skips only *its own* completed inner instances, and (3) no inner instance is mis-skipped across outer instances (the re-entering instance's accumulator equals a from-scratch run). A companion case pins (a) directly: a legacy-shaped record with no `enclosing_fan_out_lineage` for a nested fan-out forces a full re-run rather than a mis-skip. These use the existing crash-injection / resume directives (proposal 0070) and nested-fan-out modeling; no new conformance-adapter primitive is anticipated, beyond asserting the new record field.
