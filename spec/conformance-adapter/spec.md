...
- **`calls_llm_from_wrapper: {phase: pre|post, messages: [...], yield_after_call: <bool>}`** — the adapter wraps the node in a
  ...
  **The deadline is the call site, not the wrapper's return.** For `phase: pre` the wrapper does not
  ...
  The barrier is over **delivery of the event**, not over any observer's reaction to it. An observer
  that suppresses the span (`disable_llm_spans`), or a case wired with no observers at all, satisfies
  it as soon as the event has been delivered to whatever observers exist. An adapter **MUST NOT**
