...
| `output_tool_calls` | list of tool-call records \| null | The assistant message's output `tool_calls` (llm-provider §3 `ToolCall`), in typed-event-native form — each record `{id, name, arguments}` with `arguments` the parsed argument mapping. The output-side counterpart to the tool calls carried per-message within `input_messages`. Null (or empty) when the response carried no tool calls — complementary to `output_content` (null for a tool-call-only response): together they represent the full assistant output (text and/or tool calls). Carries argument values (payload) — same privacy-gating posture as `input_messages` / `output_content`. The observability §5.5.1 `openarmature.llm.output.tool_calls` span attribute (gated) and the §5.5.10 identity projections render from this field. |
