...
- `branch_name` — optional non-empty string. Populated only for events from nodes that execute inside
  ...
  no inner nodes, the branch itself is the event-source unit: it emits one `started` / `completed`
  pair carrying `branch_name`, with the branch's name as its `node_name` / `namespace`. A
  `when`-skipped branch (§11.10) emits no events.
