...
| `usage` | record \| null | Token usage record per llm-provider §6 `Response.usage` shape (including the prefix-cache fields `cached_tokens` and `cache_creation_tokens` per proposal 0047 when populated). **Mirrors the response**, which is an always-present record: 'the provider did not report usage' surfaces as a present record with the first three counters `null` together (§6), a malformed counter as a present record with that counter `null` (llm-provider §7 *Malformed usage counter*) — **not** as a `null` `usage`. The `record | null` type is retained for forward-compatibility; no current completion mapping produces a `null` `usage`. |
...
| `token_budget` | record \| null | The active prompt's token budget — same field and source as on `LlmCompletionEvent` (prompt-management §3 `Prompt.token_budget`). Null when no active prompt or no declared budget. Budget **evaluation** (the exceeded signal + §11 instruments) occurs on this variant only for `structured_output_invalid` — the failure category carrying `usage` (proposal 0082); other categories carry no usage to compare against, so the field may be populated but no evaluation occurs. See observability §5.5 / §11.2 (proposal 0083). |
...
| `response_id` | string \| null | The provider-returned response identifier when present. Mirrors the response: a malformed **or empty-string** identifier is not surfaced here either (retrieval-provider §7 *Malformed ancillary figures*; an empty string is not a present identifier). |
...
| `request_params` | mapping | Embedding-specific runtime-config fields the caller supplied (`dimensions` and `input_type` per retrieval-provider §2). Absence-is-meaningful semantics per the equivalent field on `LlmCompletionEvent`. Empty mapping when no parameters were supplied. |
...
- §6 `LlmCompletionEvent.usage` **mirrors the response** for a malformed usage counter (llm-provider §7 *Malformed usage counter*): a partially-malformed record surfaces as a present record with the malformed counter(s) `null`, an all-malformed record as a present record of null counters (§6 null-together), **not** as a null `usage` — pinning which shape a malformed-counter response takes so the event and the response agree, and closing a divergence 0093 could leave dormant (no completion mapping reported "no usage" then; a malformed all-counter response reaches that state now). The `record | null` type is unchanged. Companion to the retrieval-side `EmbeddingEvent` / `RerankEvent` mirror (0100) by [proposal 0101](../../proposals/0101-malformed-usage-counter-llm-observability.md)
