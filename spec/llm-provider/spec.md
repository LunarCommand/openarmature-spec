...
- **Additive / list-shaped** managed field — the mapping's mandatory value(s) and the caller's value(s) are
  ...
  **MUST NOT** raise or emit a diagnostic. Malformation is judged **structurally**; the mapping does **not**
  ...
  This is the request-side counterpart of §7's treatment of a malformed *ancillary* figure as not-reported: a
  malformed optional extra is treated as not-supplied, not as a request error.
- **Non-additive** managed field whose override would break the mapping's contract or its response consumer —
  ...
  **equal** to the managed value is a redundant no-op; a **conflicting** value is **rejected pre-send** with
  ...
  "Equal" and "conflicting" **MUST** be judged by **decoded-value deep equality**: the caller's and the managed
