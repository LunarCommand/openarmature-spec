...
- observability §8.4.3 — token-budget WARNING: `observation.level = "WARNING"` + a `statusMessage`
  ...
  metrics are MUST; the WARNING surfaces are SHOULD, §5.5.15). Per proposal 0109 the observer also sets
  the flat `metadata.token_budget_exceeded` boolean regardless of level (MUST).
...
1. `langfuse_generation_warning_level_on_token_budget_exceed` — Budgeted prompt (`input_max_tokens: 10`)
   ...
   exceeded: input 20 > 10"`, `metadata.token_budget.input_max_tokens = 10`, and — per proposal 0109 — the
   flat `metadata.token_budget_exceeded = true` flag, alongside the normal success rendering and the
   `metadata.prompt` identity block.
...
— a `structured_output_invalid` failure (ERROR per §8.4.2) from a budgeted prompt over budget — renders
`level = "ERROR"` (the failure wins the level / statusMessage), with the budget still on
`metadata.token_budget.*` **and** the `metadata.token_budget_exceeded` flag surviving ERROR precedence
(proposal 0109). Fixture 131 exercises the OTel metric/event side of that failure path, and fixture 155
the Langfuse exceeded-flag side.
