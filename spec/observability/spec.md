...
- Caller keys also MUST NOT exactly match any OA-emitted metadata key name that a backend
  ...
  MUST add the key name to this reserved set, unless the key already falls under one of the
  reserved namespaces above, in which case the namespace reservation covers it and a separate
  exact-match entry is redundant.
...
flowchart TB
    inv["invocation (root)"]
    inv --> outer_in["node: outer_in"]
    inv --> outer_sub["subgraph: outer_sub"]
    outer_sub --> inner_x["node: inner_x"]
    outer_sub --> inner_y["node: inner_y"]
    inv --> outer_out["node: outer_out"]
```
...
```mermaid
flowchart TB
    turn["turn span (harness)"]
    turn --> inv["invocation span (this spec, §4.1)"]
    inv --> nodes["node spans"]
    nodes --> more["..."]
```
...
fallback when that span is not the call's immediate enclosure), disambiguated by the
`openarmature.llm.attempt_index` attribute
...
node's span is **not the call's immediate enclosure** — a call issued from middleware (pre- or
...
the invocation span when a more-specific enclosing wrapper (per §4.3) **encloses the call**. This
rule is shared by the embedding (§5.5.8), tool-execution (§5.5.11), and rerank (§5.5.13) spans.
...
- `disable_provider_payload: bool` — default `True`. When `True`, payload attributes from any
  ...
  default-conservative posture preserved.)
    ...
    **Failed-call error message (proposal 0118).** The flag additionally gates a failed provider
    ...
    **`error_type` is NOT gated.** It is a classification token, a vendor error code or an upstream
    ...
    openarmature's own span (§6), which this flag does not affect. So a caller running the default
    posture (`True`) sees error types and categories in Langfuse, and full exception text in their
    OTel backend.
...
attributes. The span attributes it reaches **include** the §5.5.1 LLM attributes
(`openarmature.llm.input.messages`, `openarmature.llm.output.content`,
`openarmature.llm.output.tool_calls`, `openarmature.llm.request.extras`), the §5.5.8 embedding payload
(`openarmature.embedding.input.strings`, `openarmature.embedding.request.extras`), the §5.5.11 tool
payload (`openarmature.tool.call.arguments`, `openarmature.tool.call.result`), and the §5.5.13 rerank
payload. This list is illustrative: each section declares its own payload attributes, and a section
declaring one brings it under this contract without an entry here. Any of them MAY be
...
- a failed observation's **`error_message`** (§5.5.4, §8.4.3, §8.4.5 to §8.4.7), because the OTel surface
  defines no `error_message` span attribute (§5.5.4).

Both are subject to this contract on the same terms as the attributes above: the same configured cap, the
same default, the same truncation algorithm, and the same marker. A value **MUST NOT** escape the cap merely
because no span attribute carries it. Both values are strings, so the algorithm applies to them unchanged;
this rule does not by itself reach a structured payload whose truncated rendering a mapping does not
define.

The OTel `record_exception` path is **not** such a value. §5.5.4 keeps the exception recorded on the OTel
span full and ungated, and §6 records it verbatim; this contract does not reach it. The cap governs the
`error_message` a **backend mapping** writes, which is the Langfuse channel §8.7 describes.
...
**Per-value byte cap.** Implementations MUST enforce a maximum byte length on each
payload-classified value individually. The default cap is **65,536 bytes (64 KiB)** per
value. Implementations MUST allow the cap to be configured per observer (specific mechanism —
constructor argument, environment variable, etc. — is implementation-defined). The byte length
is measured on the UTF-8 encoding of the final string, after JSON serialization where the value
is JSON-encoded, and after inline-image redaction (below).

**Truncation algorithm.** When a value's serialized form exceeds the configured cap, the
implementation:
...
The resulting value is at most `configured_cap` bytes (may be strictly less if `N' < N` due
...
only to the overall §5.5.5 byte cap; this specification does not specify a separate
...
embedding call, parented under the calling node's span (resolved lineage-aware per the §5.5 *Lineage-resolved parent* clause, including the orphan fallback when that span is not the call's immediate enclosure).
...
| `gen_ai.usage.input_tokens` | `EmbeddingResponse.usage.input_tokens`. **Conditionally emitted** — present only when a usage record is reported; a provider that returns no usage (e.g. TEI `/embed`) omits the attribute entirely (per the §5.5.3.1 / 0047 conditional-emission convention). |
...
execution, parented under the calling node's span (resolved lineage-aware per the §5.5 *Lineage-resolved parent* clause, including the orphan fallback when that span is not the call's immediate enclosure).
...
call, parented under the calling node's span (resolved lineage-aware per the §5.5 *Lineage-resolved parent* clause, including the orphan fallback when that span is not the call's immediate enclosure).
...
| `gen_ai.usage.input_tokens` | `RerankResponse.usage.input_tokens`. **Conditionally emitted** — present only when a usage record is reported and its `input_tokens` is non-null, omitted entirely otherwise. Both the embedding (§5.5.8) and rerank spans emit `input_tokens` conditionally (a provider may report no usage record at all); rerank additionally varies on whether a *present* record carries a token count (Voyage AI does; Cohere reports search-units instead), so the attribute genuinely exercises the conditional branch (per the §5.5.3.1 / 0047 conditional-emission convention). |
...
| `openarmature.rerank.search_units` | int | The provider-reported search-units billed for this call (sourced from `RerankResponse.usage.search_units`). Conditionally emitted: present only when a usage record is reported and its `search_units` is non-null (omitted when `usage = null`). Flat namespace matches §5.5.8's `openarmature.embedding.*` convention (no `.usage.` infix). |
...
observation's exception message (§8.4.3 / §8.4.5–§8.4.7) — into whatever backend the shared provider feeds. The
...
- **Mode (b) — the implementation constructs the client (the payload-leak invariant).** When openarmature
  ...
  are:
    ...
    - the **error message** — `error_message` (and `error_type`) on a failed LLM Generation / Embedding /
      Tool / Retriever observation (§8.4.3 / §8.4.5 / §8.4.6 / §8.4.7), live when
      `disable_provider_payload=False` (§5.5.4, which gates the harvested exception text alongside the
      provider payload). Because both ride the same flag, the error message needs no separate liveness
      test: wherever the provider payload is live, so is the error message, and the arms below cover both.

  These are the harvested-content fields the §8.4.x mapping tables emit to a Langfuse observation, and the
  set is **closed by those tables** (§8.4 *Exhaustive mapping*). Harvested content the tables do **not** map —
  a stacktrace, or the exception detail on a framework-emitted observer event those tables do not render (e.g.
  the failure-isolation event's `caught_exception`, §8.4.2 / pipeline-utilities §6.3) — is **not** a further
  gated channel but **non-conforming over-emission** that **MUST NOT** be written to any Langfuse observation;
  it reaches consumers through the surface that owns it (openarmature's OTel span via `record_exception` where
  the error propagates, else the typed observer event itself). So a new emission site is either a mapped
  channel (gated here) or forbidden outright — it cannot silently become an ungated leak.
  ...
  **MUST NOT** trigger a failure), and it does **not** require enumerating a foreign provider's exporters.
  Every harvested channel is construction-determinable, because the error message rides
  `disable_provider_payload` together with the provider payload (§5.5.4). So the arms below decide the whole
  invariant:
  ...
  _Error message: gated with the provider payload._ A failed observation's `error_message` is harvested
  exception text gated by `disable_provider_payload` (§5.5.4), so it needs no separate per-emission rule. At
  the flag's default (`True`) it is never emitted at all; with the flag `False` it rides the provider-payload
  channel through the arms above, which raise where a leak is detectable, suppress where the binding cannot be
  established, and emit where the provider is isolated or the caller has accepted a shared provider. Wherever
  the message is omitted, the observation retains its `error_type` and its error **category** where one exists
  (the `observation.statusMessage` enum, §8.4.2), both of which are classifications rather than harvested text,
  and openarmature **MUST NOT** surface the exception message through `observation.statusMessage` as a
  substitute for the omitted `error_message`. A failed **Tool** observation has **no** error category (§8.4.6 /
  §5.5.12), so under omission it is discriminated by its `error_type` alone and carries no message-derived
  status text.
  ...
  **raises** where a live payload channel would leak and it can detect the binding, and **suppresses** where it
...
**except** the one case above: a harvested-payload channel is live, the caller has not accepted a shared
provider, and openarmature establishes its observations would reach a shared provider. There, and only there,
it **MUST** raise `langfuse_provider_isolation_unavailable`.
...
observation's error message) reaches a shared provider unless the caller has explicitly accepted one, raising
where a live payload channel would leak and suppressing where it cannot establish the binding; and that the
portable guarantee across implementations is "no payload-bearing observation reaches a shared provider."
...
(per §5), with one carve-out: a failed **LLM Generation** / Tool / Embedding / Retriever observation's
`error_message` (§8.4.3 / §8.4.5–§8.4.7) is harvested exception text **gated by
`disable_provider_payload`** (§5.5.4, default `True`), and when the flag permits it, still subject to
§6's payload-leak invariant like the rest of the provider payload. Where it is omitted the observation
carries its `error_type` and its error **category**, which are classifications rather than harvested text and
are not gated.

**Exhaustive mapping (no harvested over-emission).** These tables are the **complete** definition of the
**harvested content** openarmature renders to Langfuse: openarmature writes a harvested error message or
content field onto a Langfuse observation **only** where a table below maps it, and there subject to the
payload flags (§5.5.4, §8.4.1) and §6's payload-leak invariant. A harvested field that **no** §8.4.x table maps — a stacktrace, or the exception detail
carried on a framework-emitted observer event these tables do not render (for example the failure-isolation
event's `caught_exception` record, pipeline-utilities §6.3) — is **non-conforming over-emission** and **MUST
NOT** be written to any Langfuse observation. Such detail reaches consumers through the surface that owns it:
...
chain, and — when that observation is not the call's immediate enclosure — the nearest enclosing
ancestor observation per the §4.3 rule, resolved **structurally** per §5.5 rather than by which
observations the observer has materialized. Because both trees resolve the same way, the Langfuse
observation tree and the OTel span tree produce the same parent for the nested and orphan cases; that
parity is what would break if either side kept walking to the nearest *open* ancestor.
...
The `error_message` is the raised exception's string, a harvested surface **distinct** from
`generation.output`: a `structured_output_invalid` failure carries both, while every other failure category
(for example `provider_invalid_request`, a timeout, `provider_unavailable`) carries no output, so the error
message is the Generation's only harvested error content. That is why the payload flag governs both. Unlike a
Tool failure (no category, §8.4.6), a failed Generation always has an error category (§5.5.7), so it retains
its `statusMessage` when the message is omitted, and the message **MUST NOT** be surfaced through
`statusMessage` as a substitute (§6).
...
observation identified by the event's lineage chain, or the nearest enclosing ancestor when that
observation is not the call's immediate enclosure, resolved structurally per §5.5),
...
`error_type` / `error_message` in `metadata` (the §8.4.2 error row). `error_type` is not gated; `error_message`
is **gated by `disable_provider_payload`** (§5.5.4, default `True`, so the default posture carries the type and
category without the message) and, when the flag permits it, subject to §6's payload-leak invariant. The
failure observation carries no `output` (no response was received).
...
| `tool.level` / status | `DEFAULT` on `ToolCallEvent`; `ERROR` on `ToolCallFailedEvent`, with `error_type` / `error_message` in metadata. `error_type` is not gated; `error_message` is **gated by `disable_provider_payload`** (§5.5.4, default `True`) and, when the flag permits it, subject to §6's payload-leak invariant. A `Tool` failure has **no** error category (§5.5.12), so wherever the message is omitted the observation is discriminated by its `error_type` alone and carries no message-derived status text (§6 forbids substituting the message into `statusMessage`). |
...
by the event's lineage chain, or the nearest enclosing ancestor when that observation is not the call's
immediate enclosure, resolved structurally per §5.5) — and trace-level cost /
...
| `retriever.usageDetails.input` | `RerankEvent.usage.input_tokens` when populated. Omitted when the rerank call reports no usage record (`usage = null`, e.g. TEI `/rerank`) or the record's `input_tokens` is null. |
| `retriever.usageDetails.searchUnits` | `RerankEvent.usage.search_units` when populated. Omitted when the rerank call reports no usage record (`usage = null`, e.g. TEI `/rerank`) or the record's `search_units` is null. Langfuse's `usageDetails` is an open-shape mapping carrying only the usage figures the provider reported (an empty map when none); the spec defines the OA convention for the rerank-specific `searchUnits` key here. |
...
`error_message` in `metadata` (the §8.4.2 error row). `error_type` is not gated; `error_message` is **gated by
`disable_provider_payload`** (§5.5.4, default `True`, so the default posture carries the type and category
without the message) and, when the flag permits it, subject to §6's payload-leak invariant. The failure
observation carries no `output`.
...
- Sets `generation.metadata.request_extras` from `openarmature.llm.request.extras` (parsed back
  from JSON).
- On a **failed** Generation, sets `generation.metadata.error_message` from the `LlmFailedEvent`'s
  exception text (§5.5.7, §8.4.3). Proposal 0118 brought this harvested text under the same flag.

When the flag is `True` (default), `generation.input`, `generation.output`,
`generation.metadata.request_extras`, and a failed Generation's `generation.metadata.error_message`
MUST NOT be set on the Generation observation. Other fields (model, modelParameters, usage,
metadata.system, metadata.response_model, metadata.response_id, prompt linkage) continue to emit per
§8.4.3 and §8.4.4 regardless of the payload flag, as does a failed Generation's
`metadata.error_type` and its error category / `statusMessage`, which are classifications rather than
harvested text (§5.5.4, §8.4.2).

**Truncation contract.** The §5.5.5 per-value byte cap applies to the OA-attribute source
...
implementation upstream). In this **inheritance** arm the value is governed by the cap of the
observer that produced the source attribute, and the Langfuse observer does **not** re-apply its
own cap to it. That is deliberate, and it is why the direct-application arm below sources the cap
differently: an inherited value has already been truncated once, and re-truncating it against a
second cap would move the marker and misreport `M`. A value with no such source has been truncated
by nobody, so the observer that writes it applies its own. The Langfuse observer:
...
- §5.5.5 *Truncation contract* generalized from an enumerated list of `openarmature.*` span attributes to **every payload-classified value**, with the cap applied by the backend mapping that writes a value having no span-attribute source, and a note that the marker is appended to a plain string for a value that was never JSON-encoded. §8.7 gained the direct-application arm for a failed observation's `metadata.error_message` under the Langfuse observer's own cap; §5.5.4 states that a message the payload flag permits is subject to the cap. §3.4 reserved four further exact names (`error_type`, `error_message`, `token_budget`, `token_budget_exceeded`) and extended the reserved-namespace rule to `openarmature_`, closing the nine underscore-prefixed keys §8.4.5 to §8.4.7 write, with §3.4's maintenance rule reconciled so a key already under a reserved namespace needs no redundant exact-match entry. The generalization reaches the operative rules, not only the framing: the cap, the truncation algorithm and the measurement basis are stated over a **value** rather than an attribute, the OTel `record_exception` path is carved out explicitly as one §5.5.4 keeps full, and the two source-less
values are stated to be strings so the rule does not silently reach a structured payload, and the §8.7 inheritance arm now says why it sources the cap differently from the direct-application arm. New fixture 160 (five cases, gating the Embedding, Generation and Retriever arms plus a below-cap control; the Tool arm is recorded as a gap since `mock_tool` is undefined); fixture 028 gained five rejection cases by [proposal 0119](../../proposals/0119-error-message-cap-and-reserved-keys.md)
- §5.5.1's `openarmature.llm.request.extras` description dropped a language-specific configuration idiom (`extra="allow"`) that the language-agnostic rule forbids, and that stated the flat extras model llm-provider §6 now settles against. It reads "the `RuntimeConfig` extras container (the undeclared pass-through keys llm-provider §6 permits)" by [proposal 0122](../../proposals/0122-declared-field-collision-reachability.md)
- §5.5 *Lineage-resolved parent* gained **Resolution is structural**: the enclosing wrapper is determined by the call's position in the graph, not by which spans an observer has materialized when it resolves. An implementation MUST NOT select a different parent because a wrapper span does not yet exist, and MUST NOT let the parent depend on event ordering. The six span-openness conditionals that made the temporal reading available are reworded to key on structural enclosure: two inside §5.5 and one each in the §5.5.8 / §5.5.11 / §5.5.13 cross-references, plus §5.5's own retry-sibling clause. §6's synthesis trigger changes from the **first inner `started` event** to the **first event that needs the span**, stated over any event whose span resolves under §5.5 rather than over provider events, so the LLM, embedding, tool-execution and rerank spans are covered without enumeration; the start time re-anchors to the triggering event, and a fifth statement of the trigger three paragraphs below is widened to match rather than left to be read as still confining synthesis. §5.4 gains the instance-span synthesis statement the per-branch dispatch span already had, so the two kinds are governed symmetrically. §5.7's `branch_name` sourcing re-anchors to the triggering event. On the Langfuse side §8.4.8's trigger defers to §6, and the four counterparts in §8.4.3 / §8.4.5 / §8.4.6 / §8.4.7 stop routing to the nearest **open** ancestor; §8.4.3's parity promise and its temporal routing were one sentence and moved together, since rewording only the routing would have left the promise asserting a parity the two trees no longer had by [proposal 0124](../../proposals/0124-orphan-provider-span-parent-resolution.md)
