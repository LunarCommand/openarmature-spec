...
- the **primary collect channel** — the `collect_field` subgraph field seeded from `target_field` and
  collected straight back out into that same `target_field`. The collect field is *seeded from `target_field`*
  by either fan-out projection-in mechanism (§9.1): (a) an `inputs` entry keying `collect_field` to
  `target_field` (the whole parent value is copied in), or (b) `item_field` equal to `collect_field` with
  `items_field` equal to `target_field` (each element of the `target_field` list is spread in — the default
  `items_field` mode).

A round-trip into a non-round-trip-idempotent reducer (e.g. `append`) doubles the value — on the collect
channel, once per instance. A `collect_field` seeded from neither mechanism — or whose seed is a *different*
parent field — does not round-trip and does not warn on that basis.
...
A field carried into a `subgraph` branch via `inputs` and back out via `outputs` through the
same subgraph field round-trips through the parent's reducer; the graph-engine §2
...
- §9.3 *Per-instance fan-in* collect-channel round-trip condition extended to the `items_field` / `item_field` per-item seeding path (the default fan-out mode): the `projection_reducer_round_trip` warning now also fires when `item_field` equals `collect_field` and `items_field` equals `target_field`, alongside the `inputs`-seeded case 0111 named — the same doubling in the more common spelling. A `collect_field` seeded from neither mechanism (or from a different parent field) does not warn. Fixture 078 gains an item-seeded round-trip case and an item-seeded no-false-positive case (no new file); graph-engine §2 and parallel-branches §11 unaffected by [proposal 0112](../../proposals/0112-fan-out-item-seeded-collect-round-trip.md)
