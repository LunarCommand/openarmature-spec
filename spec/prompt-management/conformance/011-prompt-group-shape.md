...
charter's classifier+follow-up pair) per §10.

**Spec sections exercised:**

- §10 PromptGroup — group structure: `group_name` + ordered `members`.
- §10 N>2 support — multi-stage classification example is one of the
  enumerated patterns; the group primitive handles it under the same
  shape as N=2.
- §10 member ordering — order matches the application's intended call
...
- Member order is lost (alphabetized, sorted, etc.) — would violate
  §10's order-preservation rule.
- Empty group accepted (would mean §10's "Empty groups are
