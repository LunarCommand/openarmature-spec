...
When a backend supplies `Prompt.token_budget` (proposal 0083), it populates only the recognized
bounds (`input_max_tokens` / `total_max_tokens`) and **MUST ignore any unrecognized key** in the
sourced config: unlike `sampling`, `token_budget` has no extras mapping, so a stray key is
dropped, not carried through (proposal 0109). A **malformed or unrecognized** `token_budget`
config **MUST NOT** break the LLM call — `token_budget` is advisory / observability-only (§3): a
backend that cannot parse the config MAY drop the budget (the call proceeds with no budget, or a
fallback per the backend's own error handling), but MUST NOT propagate the parse failure into the
call. A malformed **value** for a *recognized* bound is left to the backend under this call-safety
rule (reject-to-fallback and tolerate-and-drop are both conformant, provided the call is not
broken); only the unrecognized-**key** case is normatively convergent to apply-recognized-and-ignore.
...
- `prompt_render_error` — render failed. Raised by `PromptManager.render()` when:
  ...
  *Additional Chat-prompt-specific triggers* (raised under the same `prompt_render_error`
  ...
  The error MUST expose the prompt's `name`, `version`, `label`, the `variables` mapping
...
- §11 `prompt_render_error` exposed fields named — the `variables` mapping and the `description` of the render failure (previously "the variable mapping" and "a description of the render failure"), so a `carries` fixture can field-anchor them per conformance-adapter §5.13 by [proposal 0102](../../proposals/0102-general-carries-error-field-assertion.md)
