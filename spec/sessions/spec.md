...
- **`session_load_failed`** — `SessionStore.load()` raised an unrecoverable error. The invoke MUST NOT
  proceed.
- **`session_save_failed`** — `SessionStore.save()` raised at invoke exit (or mid-invoke). The invoke's
  state was already committed at the time of save; callers see this as a post-completion error and decide
  how to handle it.
...
`openarmature.correlation_id`. Structured logs emitted during the invocation carry `session_id` as a
